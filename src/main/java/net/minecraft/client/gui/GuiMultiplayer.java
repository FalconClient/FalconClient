...
public class GuiMultiplayer extends GuiScreen implements GuiYesNoCallback {
    ...
    private Tab tab = Tab.SAVED_SERVERS;

    public enum Tab {
        IRANIAN_SERVERS("selectServer.savedServers"),
        SAVED_SERVERS("selectServer.iranianServers");
    ...
    public void initGui() {
        ...
        if (!this.initialized) {
            ...
            this.serverListSelector.reloadList(this.savedServerList);
        } else {
    ...
    public void createButtons() {
        ...
        String s = I18n.format(tab.next());
        this.buttonList.add(new GuiButton(9, this.width / 2 + 4 + 50, 10, 130, 20, s));
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.enabled) {
            GuiListExtended.IGuiListEntry entry = this.serverListSelector.func_148193_k() < 0 ? null : this.serverListSelector.getListEntry(this.serverListSelector.func_148193_k());
            switch (button.id) {
                case 1:
                ...
                case 8:
                    ...
                    break;
                case 9:
                    if (tab.equals(Tab.IRANIAN_SERVERS)) {
                        tab = Tab.SAVED_SERVERS;
                        serverListSelector.reloadList(savedServerList);
                    } else {
                        tab = Tab.IRANIAN_SERVERS;
                        serverListSelector.reloadList(iranianServerList);
                    }
                    this.buttonList.clear();
                    createButtons();
                    break;
