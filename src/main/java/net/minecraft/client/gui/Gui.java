...
public class Gui {
    ...
    public static void drawModalTexturedRectWithoutLightning(int x, int y, float u, float v, int width, int height, float textureWidth, float textureHeight) {
        ...
        worldrenderer.pos(x, y + height, 0.0D).tex(u * f, (v + (float) height) * f1)
                .color(64, 64, 64, 200).endVertex();
        worldrenderer.pos(x + width, y + height, 0.0D).tex((u + (float) width) * f, (v + (float) height) * f1)
                .color(64, 64, 64, 200).endVertex();
        worldrenderer.pos(x + width, y, 0.0D).tex((u + (float) width) * f, v * f1)
                .color(64, 64, 64, 200).endVertex();
        worldrenderer.pos(x, y, 0.0D).tex(u * f, v * f1)
                .color(64, 64, 64, 200).endVertex();
        tessellator.draw();
    }
