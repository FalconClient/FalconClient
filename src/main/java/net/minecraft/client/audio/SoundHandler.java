...
public class SoundHandler implements IResourceManagerReloadListener, ITickable {
    ...
    private void loadSoundResource(ResourceLocation location, SoundList sounds) {
        ...
        for (final SoundList.SoundEntry entry : sounds.getSoundList()) {
            String s = entry.getSoundEntryName();
            ...
            switch (entry.getSoundEntryType()) {
                case FILE:
                    ...
                    try {
                        ...
                        logger.warn(String.format("Could not load sound file %s, cannot add it to event ", location) + location, ioexception);
                        continue;
                    } finally {
                        IOUtils.closeQuietly(inputstream);
                    }

                    isoundeventaccessor = new SoundEventAccessor(new SoundPoolEntry(resourcelocation1, entry.getSoundEntryPitch(), entry.getSoundEntryVolume(), entry.isStreaming()), entry.getSoundEntryWeight());
                    break;
