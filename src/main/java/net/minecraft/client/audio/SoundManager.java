...
public class SoundManager {
    ...
    public boolean isSoundPlaying(ISound sound) {
        if (!this.loaded) {
            ...
            return s != null && (this.sndSystem.playing(s) || this.playingSoundsStopTime.containsKey(s) &&
                    this.playingSoundsStopTime.get(s) <= this.playTime);
        }
    }
    ...
    public void playSound(ISound p_sound) {
        if (this.loaded) {
            if (this.sndSystem.getMasterVolume() <= 0.0F) {
                ...
                URL url = getURLForSoundResource(p_sound.getSoundLocation());
    ...
    private static URL getURLForSoundResource(final ResourceLocation res) {
        String s = String.format("%s:%s:%s", "mcsounddomain", res.getResourceDomain(), res.getResourcePath());
